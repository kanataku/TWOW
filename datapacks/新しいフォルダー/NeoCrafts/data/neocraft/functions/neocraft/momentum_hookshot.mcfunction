# まぁまずはアイテムが必要であろう
    execute at @s run loot spawn ~ ~ ~ loot neocraft:item/monemtum_hookshot
...
# 次回以降も実行するためにレシピ没収
    recipe take @s neocraft:momentum_hookshot

# なんだこのチェストは！消えてもらおう
    clear @s chest 1

# 進捗消去
    advancement revoke @s only neocraft:neocraft/momentum_hookshot