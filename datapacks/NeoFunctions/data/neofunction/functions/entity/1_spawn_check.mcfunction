...
### HPがあるものにモブタグをつける
execute as @s[team=!NonExsitentTeam] run tag @s add mob

### HPがあり、無敵ではなく、味方でもないものに敵タグをつける
execute as @s[type=!#neofunction:safe,nbt=!{Invulnerable:1b}] run tag @s add enemy
...
execute as @s[type=#neofunction:npc] run team join green @s
